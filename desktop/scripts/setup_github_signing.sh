...
if [ -f "$ENV_FILE" ]; then
  if sh -n "$ENV_FILE" 2>/dev/null; then
    ...
    echo "warning: ignoring invalid $ENV_FILE" >&2
  fi
fi
...
env_quote() {
  # double-quote a value for safe sourcing (paths, identities with parentheses)
...

{
  ...
  echo "APPLE_CER_PATH=$(env_quote "$CER")"
  echo "APPLE_PRIVATE_KEY_PATH=$(env_quote "$KEY")"
  echo "APPLE_P12_PATH=$(env_quote "$P12_OUT")"
  echo "APPLE_CERTIFICATE_PASSWORD=$(env_quote "$P12_PASSWORD")"
  echo "APPLE_SIGNING_IDENTITY=$(env_quote "$IDENTITY")"
  echo "APPLE_API_KEY_PATH=$(env_quote "$API_KEY")"
  echo "APPLE_API_KEY_ID=$(env_quote "$API_KEY_ID")"
  echo "APPLE_API_ISSUER=$(env_quote "$API_ISSUER")"
  if [ -n "$WINDOWS_PFX" ]; then
    echo "TERMX_WINDOWS_PFX=$(env_quote "$WINDOWS_PFX")"
    echo "TERMX_WINDOWS_PFX_PASSWORD=$(env_quote "${WINDOWS_PFX_PASSWORD:-}")"
  fi
} > "$ENV_FILE"
chmod 600 "$ENV_FILE"
echo "==> wrote $ENV_FILE"
